...
const byte ploadPin165        = 6;    //Conecta ao pino 1 do 74HC165 (LH/LD - asynchronous parallel load input)(PL)
const byte clockEnablePin165  = 5;    //Conecta ao pino 15 do 74HC165 (CE - Clock Enable Input)(CE)
const byte dataPin165         = 8;   //Conecta ao pino 9 do 74HC165 (Q7 - serial output from the last stage)(Q7)
const byte clockPin165        = 7;   //Conecta ao pino 2 do 74HC165 (CP - Clock Input)(CP)
...
const byte clockPin595 = 2; //Pino conectado a SRCLK (pino 11 no 74HC595), registrador de deslocamento
const byte latchPin595 = 3; //Pino conectado a RCLK (pino 12 no 74HC595), registrador de armazenamento
const byte dataPin595 = 4; //Pino conectado a SER (pino 14 no 74HC595), entrada de dados serial
...
void read_shift_regs(){
    ...
    for(int i = 0; i < nCIs; i++){
        ...
        //condição para desligar apos um tempo que o botao ficar pressionado
        if((pinValues[nCIs-1-i] != 0) && (pinKey[nCIs-1-i] == 0)){
            time = millis();
            pinKey[nCIs-1-i] = pinValues[nCIs-1-i];
        }

        //condicao para impedir o desligamento com o botao sem pressionar e resetar o pinKey
        if(pinValues[nCIs-1-i] == 0){
            pinKey[nCIs-1-i] = 0;
        }

        //condiçao para funcionar apenas com o botao 04
        if(((millis() - time) > 2000) && (pinKey[nCIs-1-i] == 8) && ((nCIs-1-i) == 0)){
            Serial.print("\nElton\n");
            Serial.print(pinValuesOut[nCIs-1-i]);
            pinValuesOut[nCIs-1-i] &= (0 << 4);
            Serial.print("\n");
            Serial.print(pinValuesOut[nCIs-1-i]);
            Serial.print("\n");
        }
        ...
        //desliga as saidas
        if(((millis() - time) > 4000) && (pinKey[nCIs-1-i] != 0)){
            pinValuesOut[nCIs-1-i] = 0;
        }
    }
}
