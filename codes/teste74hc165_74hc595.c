...
BYTES_VAL_T pinValuesOut;
BYTES_VAL_T oldPinValuesOut;
...
void read_shift_regs()
{
    ...
    for(int i = 0; i < nExtIn; i++)
    {
        ...
        pinValues |= (bitVal << ((nExtIn-1) - i));

        pinValuesOut ^= (bitVal << ((nExtIn-1) - i));
...
void alteraSaida(){
	...
	for (int i = 0; i < nCIs; ++i){
    	...
    	shiftOut(dataPin595, clockPin595, LSBFIRST, pinValuesOut >> (8*i));
...
void setup()
{
    ...
    read_shift_regs();
    pinValuesOut = pinValues;
    display_pin_values();
    oldPinValues = pinValues;
    oldPinValuesOut = pinValues;
...
void loop(){
    ...
    if (oldPinValuesOut != pinValuesOut){
    	alteraSaida();
    	oldPinValuesOut = pinValuesOut;
    }
